...
interface OutboundFlightsProps {
  flights: Flight[]
}

export function OutboundFlights({ flights }: OutboundFlightsProps) {
  const [selectedFlightId, setSelectedFlightId] = useState<string | null>(null)
  const t = useTranslations()

  useEffect(() => {
    const storedFlight = sessionStorage.getItem("selectedOutboundFlight")
    if (storedFlight) {
      try {
  ...
  const handleSelectFlight = (flightId: string) => {
    ...
    if (selectedFlight) {
      console.log(t("flights.savingSelectedOutboundFlight"), selectedFlight)
      sessionStorage.setItem("selectedOutboundFlight", JSON.stringify(selectedFlight))

      const event = new Event("outboundFlightSelected")
